...
> attr(x, "dim") <- c(2, 4) # 设置 dim 属性
> print(x)
     [,1] [,2] [,3] [,4]
[1,]    1    3    5    7
[2,]    2    4    6    8
> class(x) # 设置 dim 属性后，从变量变为矩阵
[1] "matrix" "array" 
> attributes(x) # 列出属性
$dim
[1] 2 4
...
| 函数               | 用途                                                         | 示例                                                         |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `attr()`           | 设置或修改对象的单个指定属性                                 | `attr(x, "comment") <- "备注信息"`                           |
| `attributes()`     | 设置或替换对象的所有属性（以命名列表形式）                   | `attributes(x) <- list(names = c("a","b"), class = "myclass")` |
| `mostattributes()` | 安全地批量设置属性，仅设置合法的属性（如 `dim`、`names`、`dimnames`），不会因无效属性报错 | `mostattributes(x) <- list(dim = 3:2, dimnames = list(...))` |
| `structure()`      | 在创建对象的同时设置多个属性，返回带有指定属性的对象         | `structure(1:6, dim = c(2,3), names = letters[1:6])`         |

### 特定属性专用函数 

| 函数          | 设置的属性                                       | 示例                              |
| ------------- | ------------------------------------------------ | --------------------------------- |
| `names()`     | 设置向量、列表或数据框的元素/列名称              | `names(x) <- c("a", "b", "c")`    |
| `class()`     | 设置对象的类（class）                            | `class(x) <- "data.frame"`        |
| `unclass()`   | 移除对象的 `class` 属性，使其回退为基础类型      | `unclass(x)`                      |
| `dim()`       | 设置矩阵或数组的维度                             | `dim(x) <- c(2, 3)`               |
| `dimnames()`  | 设置矩阵或数组的维度名称（行名和列名）           | `dimnames(x) <- list(rows, cols)` |
| `row.names()` | 设置矩阵或数据框的行名（底层函数）               | row.names(x) <- c("R1", "R2")     |
| `rownames()`  | 设置矩阵或数据框的行名(`row.names()` 的便捷封装) | `rownames(x) <- c("R1", "R2")`    |
| `colnames()`  | 设置矩阵或数据框的列名                           | `colnames(x) <- c("C1", "C2")`    |
| `levels()`    | 设置因子（factor）的水平值                       | `levels(x) <- c("A", "B", "C")`   |
| `tsp()`       | 设置时间序列的时间戳参数（起点、频率、终点）     | `tsp(x) <- c(2020, 2025, 1)`      |
| `comment()`   | 设置对象的注释信息                               | `comment(x) <- "这是注释"`        |

示例：以 iris 数据集为例

- data.frame 本质是 `class` 属性为 `data.frame` 的 list
- `names` 属性包含变量名称
- `row.names` 标识每个样本
